...
<head>
    ...
    <title>Edit Lembar Penilaian - {{ $guru->user->name }}</title>
...
<body>
            ...
            <div class="col-md-9 col-lg-10 p-4" style="background-color: #ffffff !important; background: #ffffff !important;">
                ...
                <div class="mb-4">
                    <h2 class="mb-2">
                        ...
                        Edit Lembar Penilaian
                    </h2>
                    <p class="text-muted mb-3">Ubah informasi lembar penilaian</p>
                    <a href="{{ route('guru.evaluasi.lembar.index') }}" class="btn btn-secondary btn-sm">
                ...
                <div class="row">
                    <div class="col-md-10 col-lg-8">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{ route('guru.evaluasi.lembar.update', $lembar->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" id="mata_pelajaran" name="mata_pelajaran" required>
                                                ...
                                                @foreach($mataPelajaranList as $mapel)
                                                    <option value="{{ $mapel->mata_pelajaran }}" {{ $lembar->mata_pelajaran == $mapel->mata_pelajaran ? 'selected' : '' }}>{{ $mapel->mata_pelajaran }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" id="kelas" name="kelas" required>
                                                ...
                                                <option value="7" {{ $lembar->kelas == '7' ? 'selected' : '' }}>Kelas 7</option>
                                                <option value="8" {{ $lembar->kelas == '8' ? 'selected' : '' }}>Kelas 8</option>
                                                <option value="9" {{ $lembar->kelas == '9' ? 'selected' : '' }}>Kelas 9</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" id="semester" name="semester" required>
                                                ...
                                                <option value="Ganjil" {{ $lembar->semester == 'Ganjil' ? 'selected' : '' }}>Ganjil</option>
                                                <option value="Genap" {{ $lembar->semester == 'Genap' ? 'selected' : '' }}>Genap</option>
                                            </select>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="tanggal_penilaian" class="form-label">Tanggal Penilaian <span class="text-danger">*</span></label>
                                            <input type="date" class="form-control" id="tanggal_penilaian" name="tanggal_penilaian" required value="{{ $lembar->tanggal_penilaian }}">
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <label for="rubrik_penilaian_id" class="form-label">Rubrik Penilaian / Kuis</label>
                                        <select class="form-select" id="rubrik_penilaian_id" name="rubrik_penilaian_id">
                                            <option value="">Pilih Rubrik atau Kuis (Opsional)</option>
                                            @if($rubrikList->count() > 0)
                                                <optgroup label="Rubrik Penilaian">
                                                    @foreach($rubrikList as $rubrik)
                                                        <option value="{{ $rubrik->id }}" {{ $lembar->rubrik_penilaian_id == $rubrik->id ? 'selected' : '' }}>{{ $rubrik->judul }}</option>
                                                    @endforeach
                                                </optgroup>
                                            @endif
                                            @if($kuisList->count() > 0)
                                                <optgroup label="Kuis">
                                                    @foreach($kuisList as $kuis)
                                                        <option value="kuis_{{ $kuis->id }}">{{ $kuis->judul }}</option>
                                                    @endforeach
                                                </optgroup>
                                            @endif
                                        </select>
                                        <small class="text-muted">Pilih rubrik penilaian atau kuis yang terkait dengan penilaian ini</small>
                                    </div>

                                    <div class="mb-3">
                                        <label for="nilai" class="form-label">Nilai</label>
                                        <input type="number" step="0.01" min="0" max="100" class="form-control" id="nilai" name="nilai" value="{{ $lembar->nilai }}" placeholder="0.00 - 100.00">
                                    </div>

                                    <div class="mb-3">
                                        <label for="aspek_penilaian" class="form-label">Aspek Penilaian</label>
                                        <textarea class="form-control" id="aspek_penilaian" name="aspek_penilaian" rows="3">{{ $lembar->aspek_penilaian }}</textarea>
                                    </div>

                                    <div class="mb-3">
                                        <label for="catatan" class="form-label">Catatan</label>
                                        <textarea class="form-control" id="catatan" name="catatan" rows="3">{{ $lembar->catatan }}</textarea>
                                    </div>
    ...
    <script>
        ...
        const selectedSiswaId = {{ $lembar->siswa_id ?? 'null' }};
