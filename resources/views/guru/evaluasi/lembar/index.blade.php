...
<head>
    ...
    <title>Lembar Penilaian - {{ $guru->user->name }}</title>
        ...
        <div class="row" style="background-color: #ffffff !important; background: #ffffff !important;">
            ...
            <div class="col-md-9 col-lg-10 p-4" style="background-color: #ffffff !important; background: #ffffff !important;">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h2 class="mb-1">Lembar Penilaian (LP)</h2>
                        <p class="text-muted mb-0">Kelola lembar penilaian untuk setiap siswa</p>
                    </div>
                </div>

                @if(!$tableExists)
                    <div class="alert alert-warning">
                        <h5><i class="fas fa-exclamation-triangle me-2"></i>Tabel Lembar Penilaian Belum Ada</h5>
                ...
                @else
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <a href="{{ route('guru.evaluasi.lembar.create') }}" class="btn btn-primary">
                                <i class="fas fa-plus me-2"></i>Tambah Lembar Penilaian
                            </a>
                        ...
                        <div class="col-md-6">
                            <form method="GET" action="{{ route('guru.evaluasi.lembar.index') }}" class="d-flex gap-2">
                                ...
                                <tbody>
                                    @foreach($lembar as $item)
                                        <tr>
                                            ...
                                            <td>
                                                @if($item->nilai)
                                                    <span class="badge bg-primary">{{ number_format($item->nilai, 2) }}</span>
                                                @else
                                            ...
                                            <td>
                                                <div class="d-flex gap-2">
                                                    <a href="{{ route('guru.evaluasi.lembar.show', $item->id) }}" class="btn btn-sm btn-info">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a href="{{ route('guru.evaluasi.lembar.edit', $item->id) }}" class="btn btn-sm btn-warning">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form action="{{ route('guru.evaluasi.lembar.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    @else
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>Belum ada lembar penilaian. <a href="{{ route('guru.evaluasi.lembar.create') }}">Buat lembar penilaian pertama</a>
                        </div>
                    @endif
                @endif
            </div>
