@echo off
title FIX OTOMATIS - JALANKAN INI SAJA
color 0A

echo.
echo ============================================
echo FIX OTOMATIS - JALANKAN INI SAJA
echo ============================================
echo.
echo Script ini akan melakukan SEMUA konfigurasi
echo otomatis. TIDAK PERLU SETTING MANUAL!
echo.
...
echo [LANGKAH 2] Mengaktifkan mod_rewrite...
powershell -Command "$file = '%XAMPP_PATH%\apache\conf\httpd.conf'; if (Test-Path $file) { $content = Get-Content $file -Raw; $content = $content -replace '#LoadModule rewrite_module modules/mod_rewrite.so', 'LoadModule rewrite_module modules/mod_rewrite.so'; Set-Content $file -Value $content -NoNewline }" >nul 2>&1
echo [OK] mod_rewrite diaktifkan.
echo.

echo [LANGKAH 3] Membuat certificate SSL...
...
echo [LANGKAH 4] Mengaktifkan mod_ssl...
...
echo [LANGKAH 5] Membuat VirtualHost...
set "VHOST_PATH=%XAMPP_PATH:\=/%"
powershell -ExecutionPolicy Bypass -Command "$xampp = '%XAMPP_PATH%'; $vhostPath = '%VHOST_PATH%'; $vhostsFile = \"$xampp\apache\conf\extra\httpd-vhosts.conf\"; $vhost = \"`n# VirtualHost untuk nuranitms.test`n<VirtualHost *:80>`n    ServerName nuraniTMS.test`n    ServerAlias www.nuraniTMS.test nuranitms.test www.nuranitms.test`n    DocumentRoot `\"$vhostPath/htdocs/nurani/public`\"`n    RewriteEngine On`n    RewriteCond %{HTTPS} off`n    RewriteRule ^(.*)$ https://%{HTTP_HOST}%{REQUEST_URI} [L,R=301]`n</VirtualHost>`n`n<VirtualHost *:443>`n    ServerName nuraniTMS.test`n    ServerAlias www.nuraniTMS.test nuranitms.test www.nuranitms.test`n    DocumentRoot `\"$vhostPath/htdocs/nurani/public`\"`n    SSLEngine on`n    SSLCertificateFile `\"$vhostPath/apache/conf/ssl/nuraniTMS.crt`\"`n    SSLCertificateKeyFile `\"$vhostPath/apache/conf/ssl/nuraniTMS.key`\"`n    SSLProtocol all -SSLv2 -SSLv3`n    SSLCipherSuite HIGH:!aNULL:!MD5`n    <Directory `\"$vhostPath/htdocs/nurani/public`\">`n        Options Indexes FollowSymLinks`n        AllowOverride All`n        Require all granted`n    </Directory>`n</VirtualHost>`n\"; if (Test-Path $vhostsFile) { $content = Get-Content $vhostsFile -Raw; if ($content -notmatch 'nuraniTMS\.test') { Add-Content $vhostsFile -Value $vhost } }" >nul 2>&1
echo [OK] VirtualHost dibuat.
echo.

echo [LANGKAH 6] Update file hosts...
