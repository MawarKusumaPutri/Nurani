@echo off
echo ========================================
echo   FIX TABLESPACE ERROR - PASTI BERHASIL
echo ========================================
echo.
echo [INFO] Script ini akan:
echo 1. Discard tablespace via SQL
...
echo [2/4] Mencoba discard tablespace dan hapus database...
php -r "try { $pdo = new PDO('mysql:host=127.0.0.1;port=3306', 'root', ''); $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); try { $pdo->exec('USE nurani'); try { $pdo->exec('ALTER TABLE migrations DISCARD TABLESPACE'); echo '[INFO] Tablespace sudah di-discard.\n'; } catch(Exception $e) { echo '[INFO] Tidak bisa discard tablespace (mungkin tabel tidak ada): ' . $e->getMessage() . '\n'; } } catch(Exception $e) { echo '[INFO] Database tidak ada atau tidak bisa diakses.\n'; } $pdo->exec('DROP DATABASE IF EXISTS nurani'); echo '[SUKSES] Database sudah dihapus (jika ada)!\n'; } catch(Exception $e) { echo '[INFO] ' . $e->getMessage() . '\n'; }"
echo.
...
if %errorlevel% equ 0 (
    ...
    echo.
    echo [SOLUSI ALTERNATIF] Jika masih error, coba:
    echo 1. Stop MySQL di XAMPP
    echo 2. Hapus folder: C:\xampp\mysql\data\nurani (jika ada)
    echo 3. Start MySQL di XAMPP
